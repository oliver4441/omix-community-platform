...
import { PublicFlowStyles } from "@/components/ui/PublicFlowStyles";
...
function detectAuthLink(): AuthLink | null {
  ...
  return token && (type === "verify" || type === "recovery" || type === "reset") ? { token, type } : null;
}

function clearAuthLinkParams() {
  try { const url = new URL(window.location.href); url.search = ""; url.hash = ""; window.history.replaceState({}, document.title, url.toString()); } catch {}
}
...
function AppInner() {
  ...
  useEffect(() => {
    ...
    api.auth.verifyEmail(authLink.token).then(() => setVerifyDone(true)).catch(() => setVerifyError("This verification link is invalid or has expired."));
  }, [authLink, verifyDone]);

  useEffect(() => {
    if (!user) return;
    const timer = setTimeout(() => Store.requestNotificationPermission(), 5000);
  ...
  if (authLink?.type === "verify") {
    if (verifyError) return <VerifyEmailScreen email={verifyEmail || ""} error={verifyError} onChangeEmail={() => { setAuthLink(null); clearAuthLinkParams(); setFlow("signup"); }} />;
    if (!verifyDone) return <LoadingScreen />;
    return <VerificationSuccessScreen onLaunch={() => { setAuthLink(null); clearAuthLinkParams(); setFlow("signin"); }} />;
  }

  if (!user) {
    switch (flow) {
      case "forgot": return <ForgotPasswordScreen onBack={() => setFlow("signin")} />;
      case "verify": return <VerifyEmailScreen email={verifyEmail} onChangeEmail={() => setFlow("signup")} />;
      case "signin":
      case "signup":
...
export default function App() {
  return <AuthProvider><ToastProvider><ConfirmProvider><SWRegister /><AppInner /></ConfirmProvider></ToastProvider></AuthProvider>;
}
